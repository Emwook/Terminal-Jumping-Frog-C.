...
void setObjectSizeAndCenter(object_t *o, position_t pos, position_t size) {
    ...
    readArtFromFile("default_input.txt", o);
}
...
void placeObject(board_t *b, object_t *obj) {
    ...
    // Calculate the size
    const int sizeY = is_full_length ? b->size : obj->model.size.y; // Use board size for full-length objects
    const int sizeX = obj->model.size.x; // Use board size for full-length objects
    ...
    for (int i = 0; i < sizeX; i++) {
        for (int j = 0; j < sizeY; j++) {
            ...
            if (is_full_length) {
                artY = j % obj->model.size.y;  // Wrap horizontally based on object width
...
void moveObject(board_t *b, object_t *o, char direction) {
    position_t stepBy = {4,4}; // change to dynamic stuff
    for (int i = 0; i < o->model.size.x; i++) {
...
int main() {
    ...
    // position_t frogStartPos = {4, SIZE/2};
    ...
    //     drawVisualLayer(&board);
    //     // drawGameplayLayer(&board); // currently there is some frog spillage in the gameplay layer
    //     repeat--;
    // }

    for (int i = 0; i < frog.model.size.y; i++) {
        free(frog.model.art[i]);
    }
    free(frog.model.art);
    ...
    for (int i = 0; i < lane.model.size.y; i++) {
    free(lane.model.art[i]);
    }
    free(lane.model.art);

    for (int i = 0; i < car.model.size.y; i++) {
        free(car.model.art[i]);
    }
    free(car.model.art);
...
// errors with memory allocation an stuff