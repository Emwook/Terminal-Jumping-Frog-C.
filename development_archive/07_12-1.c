...
void moveCars(board_t *board, int time);
...
void initializeMovingCars(moving_cars_set_t *cars, int amount) {
    ...
    for (int i = 0; i < amount; i++) {
        ...
        // cars->objects[i].object.center.x = -1;
        cars->objects[i].object.center.y = -1;
        cars->objects[i].direction = 'd';
        cars->objects[i].speed = 1;
        cars->objects[i].timeout_until = -1;
    ...
    for (int i = 0; i < sizeX; i++) {
        for (int j = 0; j < sizeY; j++) {
            ...
            if (posX > 0 && posX < b->size -1 && posY > 0 && posY < b->size -1) {
                ...
                if(symbol == 'l' ){
                    b->gameplay_layer[posX][posY] = (obj->model.art[artX][artY] !=' ')?symbol:' ';
                }
                else{
...
void getCarsOnLane(position_t *cars_on_lane, board_t *board, int direction, int laneIndex, int *number_of_cars) {
    for (int i = 0; i < board->cars.amount; i++) {
        if (board->cars.objects[i].timeout_until == -1 &&
...
void processCarObject(board_t *b, model_t *car_m, position_t car_start_pos, char direction, int *cars_to_place, int laneIndex) {
    for (int i = 0; i < b->cars.amount; i++) {
        if (b->cars.objects[i].object.center.x <=0 && b->cars.objects[i].object.center.y <=0) {
            ...
            b->cars.objects[i].timeout_until =(-1);
...
void placeCarOnLane(board_t *b, model_t *car_m, int laneIndex, int *cars_to_place, char direction) { //   this please
    ...
    if (cars_in_lane < max_cars_per_lane) {
        ...
        //board_t *b, model_t *car_m, int laneIndex, int *cars_to_place, char direction, int max_cars_per_lane, int cars_in_lane, int direction_sign, position_t car_start_pos,
...
void printScoreboard( scoreboard_t *s) {
    ...
    float max_game_time_seconds = s->max_game_time / 1000.0;

    // int rounded_game_time = (game_time_seconds * 4 + 2) / 4;  // rounding to nearest quarter second
    // int rounded_max_game_time = (max_game_time_seconds * 4 + 2) / 4;
...
void checkFrogCollision(board_t *board, scoreboard_t *scoreboard) {
    int frogX = board->frog.center.x;
    int frogY = board->frog.center.y;
    int sizeX = board->frog.model.size.x / 2;
    int sizeY = board->frog.model.size.y / 2;
    int radius = 1;
...
// void checkCarFrontCollision(board_t *board) {    
//     for (int i = 0; i < board->cars.amount; i++) {
//         object_t *car = &board->cars.objects[i].object;
//         int carX = car->center.x;
//         int carY = car->center.y;
//         int carSizeX = car->model.size.x;
//         int carSizeY = car->model.size.y;
        
//         char direction = board->cars.objects[i].direction;

//         int checkOffset = (direction == 'd') ? 1 : -1;
//         int offset = 1;
//         int checkX = carX + checkOffset * (carSizeX + offset);
//         if (checkX >= 0 && checkX < board->size) {
//             for (int j = 0; j < board->cars.amount; j++) {
//                 if (j != i) {
//                     object_t *otherCar = &board->cars.objects[j].object;
//                     if (otherCar->center.x == checkX && otherCar->center.y == carY) {
//                         if (board->cars.objects[i].speed > 1) {
//                             board->cars.objects[i].speed--;
//                         }
//                         return;
//                     }
//                 }
...
void getInputAndProcess(char *move) {
    // Initialize curses
    initscr();
    timeout(0);  // Set non-blocking mode (getch won't wait for Enter)
    cbreak();    // Disable line buffering
    noecho();    // Don't echo the input to the screen
    keypad(stdscr, TRUE); // Enable keypad input if necessary

    char input_char = getch();  // Get single character input

    if (input_char != ERR) {  // Check if a character was input
        *move = input_char;  // Pass it to the function for processing
    }
...
void moveCars(board_t *board, int time) {
    for (int i = 0; i < board->cars.amount; i++) {
        int left_edge = board->cars.objects[i].object.center.y + board->cars.objects[i].object.model.size.y;
        int right_edge = board->cars.objects[i].object.center.y - board->cars.objects[i].object.model.size.y;
        if(board->cars.objects[i].timeout_until ==(-1)){
            ...
            int car_amount = 1;
            placeCarOnLane(board, &(board->cars.objects[i].object.model), (board->cars.objects[i].lane_number), &car_amount, board->cars.objects[i].direction);
        }
    }
}
...
int main() {
    ...
    const int MAX_GAME_TIME = 20000;
    const int AMOUNT_OF_CARS = 9;
    ...
    scoreboard.max_game_time = MAX_GAME_TIME;
    ...
    createCarsOnLanes(&board, &car_right_model, (AMOUNT_OF_CARS/2 + AMOUNT_OF_CARS%2), 'd');
    ...
    while (strcmp(scoreboard.game_state, "in game") == 0) {
        ...
        scoreboard.game_time += 100;
        char moveDirection=' ';
        getInputAndProcess(&moveDirection);
        if(moveDirection != ' '){
            moveObject(&board, &(board.frog), moveDirection);
        }
        moveCars(&board, scoreboard.game_time);
        // checkCarFrontCollision(&board); // not like this
        refreshBoard(&board);
        drawVisualLayer(&board);
        drawGameplayLayer(&board);
        // printCarsPositions(&board);
        checkFrogCollision(&board, &scoreboard);
        if(scoreboard.game_time  >= scoreboard.max_game_time){
            scoreboard.game_state = "game over";
        }
        printScoreboard(&scoreboard);
        usleep(100000);
