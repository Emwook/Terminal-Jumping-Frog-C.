...
void createCarsOnLanes(board_t *b, model_t *car_m, int cars_amount, char direction, const int MAX_SPEED);
...
// a bit buggy in current form i must say
// void checkCollision(board_t *board, scoreboard_t *scoreboard){
...
//     int radius = 1;
//     // bus error beep beep - new error unlocked hurray
//     for (int i = -1; i <= 1; i+=2) {
//         for (int j = -1; j <= 1; j+=2) {
//             int to_check_x = (frogX+i*(sizeX+radius)<=board->size && frogX+i*(sizeX+radius)>=0)?(frogX+i*(sizeX+radius)):(frogX);
//             int to_check_y = (frogY+i*(sizeY+radius)<=board->size && frogY+i*(sizeY+radius)>=0)?(frogY+i*(sizeY+radius)):(frogY);
//             if (board->gameplay_layer[frogX+i*(sizeX+1)][frogY+i*(sizeY+1)] == 'c') {
//                 strcpy(scoreboard->game_state, "game over");
//                 return;
//             }
//         }
//     }
...
void printCarsPositions(const board_t *b) {
    ...
    printf("Cars (total: %d) :\n", b->cars.amount);
    for (int i = 0; i < b->cars.amount; i++) {
        printf("Car %d: (%d, %d) %s %d \n", i + 1, b->cars.objects[i].object.center.x, b->cars.objects[i].object.center.y, b->cars.objects[i].object.name, b->cars.objects[i].speed);
    }
}
...
int main() {
    ...
    createCarsOnLanes(&board, &car_right_model, AMOUNT_OF_CARS/2, 'd', MAX_SPEED);
    createCarsOnLanes(&board, &car_left_model, AMOUNT_OF_CARS/2, 'a', MAX_SPEED);
    ...
    while (strcmp(scoreboard.game_state, "in game") == 0) {
        ...
        // checkCollision(&board, &scoreboard);
