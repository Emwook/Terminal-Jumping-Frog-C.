// segmentation fault in this file -- not working
...
void createObject(board_t *b, object_t *obj); // places an object on both game and visual layers, repeats the object if necessery
...
void createObject(board_t *b, object_t *obj) {

    const char symbol = (obj->name[0])?(obj->name[0]):(' '); //error
...
void moveObject(board_t *b, object_t *o, char direction) {
    ...
    // clear the model from board first
    for (int i = 0; i < o->model.size.x; i++) {
...
void initializeMovingObjects(moving_object_set_t *cars, int amount) {
    ...
    for (int i = 0; i < amount; i++) {
        cars->objects[i].object.intent = 'n'; // Default intent
...
void checkCollision(board_t *board, moving_object_t *mo){
    const position_t stepBy = {1,1};
    char intent = mo->object.intent;
    switch (mo->direction) {
        case 'w':
            if ((intent == 'e')||(mo->object.center.x - mo->object.model.size.x / 2 > 2)) 
                mo->object.center.x -= stepBy.x;
            break;
        case 's':
            if (mo->object.center.x + mo->object.model.size.x / 2 < board->size-3) 
                mo->object.center.x += stepBy.x;
            break;
        case 'a':
            if ((intent == 'e')||mo->object.center.y - mo->object.model.size.y / 2 > 4) 
                mo->object.center.y -= stepBy.y;
            break;
        case 'd':
            if (mo->object.center.x + mo->object.model.size.x / 2 < board->size - 3) 
                mo->object.center.y += stepBy.y;
...
int main() {
    ...
    // createObject(&board, &car_right);
    // createObject(&board, &car_left);
    ...
    while (repeat > 0) {
        ...
        moveCars(&board); //segmentation fault here
        repeat--;
    }
