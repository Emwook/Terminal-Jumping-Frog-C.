...
void moveObject(board_t *b, object_t *o, char direction, position_t step) {
    for (int i = 0; i < o->size.x; i++) {
        for (int j = 0; j < o->size.y; j++) {
            int posX = o->center.x - o->size.x / 2 + i;
            int posY = o->center.y - o->size.y / 2 + j;
    ...
    switch (direction) {
    case 'w':
        if (o->center.x - o->size.x / 2 > 2) 
            o->center.x -= step.x;
        break;
    case 's':
        if (o->center.x + o->size.x / 2 < b->size-3) 
            o->center.x += step.x;
        break;
    case 'a':
        if (o->center.y - o->size.y / 2 > 4) 
            o->center.y -= step.y;
        break;
    case 'd':
        if (o->center.y + o->size.y / 2 < b->size - 3) 
            o->center.y += step.y;
        break;
    default:
        break;
...
void createMultipleLanes(board_t *b, object_t *lane, int max_lanes) {
    int lanes_amount = (b->size - 2) / (lane->size.x * 1.5); 
    lanes_amount = lanes_amount > max_lanes ? max_lanes : lanes_amount;

    for (int i = 0; i < lanes_amount; i++) {
        position_t laneStartPos = {(i + 1) * (1.4*lane->size.x), b->size / 2};
...
int main() {
    ...
    while (repeat > 0){
        ...
        createMultipleLanes(&board, &lane, MAX_LANES);
        placeObject(&board, &car);
        moveObject(&board, &frog, 'w', frog.size);
        moveObject(&board, &car, 'd', car.size);
        drawVisualLayer(&board);
    ...
    for (int i = 0; i < lane.size.y; i++) {
    free(lane.art[i]);
    }
    free(lane.art);

    for (int i = 0; i < car.size.y; i++) {
        free(car.art[i]);
    }
    free(car.art);
